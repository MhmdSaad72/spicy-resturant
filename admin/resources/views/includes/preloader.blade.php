<!-- Preloader -->
<div class="preloader">
  <div class="preloader-inner">
    <div class="position-relative text-center"><span class="bg-heading-text-content center">Loading</span>
      <p class="h4 mb-2 d-flex align-items-center"> <span>{{isset($basicDetail->name) ? $basicDetail->name : ''}}</span><i class="fas fa-hamburger fa-2x px-3 text-primary"></i><span>Restaurant</span></p>
      <p class="text-small font-weight-light text-gray mb-0">We're loading our awesome stuff</p>
    </div>
    <div class="lds-ellipsis">
      <div></div>
      <div></div>
      <div></div>
      <div></div>
    </div>
  </div>
</div>
